...
def generate_events(distribution, num_events):
...
def main():
    ...
    for trial in range(NUM_TRIALS):
        ...
        # 📊 Print total counts
        print(f"Total spin-up events: {len(phi_up)} (Left: {len(phi_up_left)}, Right: {len(phi_up_right)})")
        print(f"Total spin-down events: {len(phi_down)} (Left: {len(phi_down_left)}, Right: {len(phi_down_right)})")
